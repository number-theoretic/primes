...
test( "ulam convert" , t => {
	...
	t.is( atos( 0 , 4 ) , 2 * 4 + 1 , "1 should be in the upper left of the center ( n = 4 )" ) ;
	t.is( atos( 0 , 5 ) , 2 * 5 + 2 , "1 should be in the center ( n = 5 )" ) ;

	t.is( stoa( 0 * 1 + 0 , 1 ) , 0 , "the only element should be 1 ( n = 1 )" ) ;
	t.is( stoa( 1 * 2 + 0 , 2 ) , 0 , "the upper left corner should contain 1 ( n = 2 )" ) ;
	t.is( stoa( 1 * 3 + 1 , 3 ) , 0 , "the center should contain 1 ( n = 3 )" ) ;
	t.is( stoa( 2 * 4 + 1 , 4 ) , 0 , "the upper left of the center should contain 1 ( n = 4 )" ) ;
